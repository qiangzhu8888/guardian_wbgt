      ...
      <div className="space-y-10 max-w-3xl">
        ...
        <ManualSection id="facilities" title="4. 場所（監視地点）">
          ...
          <ul className="list-disc list-outside pl-5 space-y-2">
            ...
            <li>
              ...
              まで）を登録できます。Firebase Storage が有効である必要があります。監視ダッシュボードでは施設名の横に正方形のサムネイルとして表示されます（一辺およそ <strong>58px</strong>／従来比おおよそ 120%）。詳細画面では横幅いっぱい・高さは最大およそ{' '}
              <strong>270px（従来比おおよそ 120%）</strong>
              で表示されます（画像の縦横比ははみ出しを切り詰める形で収まります）。
            </li>
            <li>
        ...
        <ManualSection id="monitoring-public" title="7. 公開監視画面（エンドユーザー向け・確認）">
          <p>
            ...
            で開きます。ダッシュボードでは場所ごとに、<strong>
              現場センサーに基づく現在の WBGT（暑さ指数の推定値）
            </strong>
            と気温・湿度などが表示されます。この数値と別に、緯度・経度から取得する{' '}
            <strong>日本気象協会（JWA）の 1km メッシュによる時別 WBGT 予測（参考）</strong>
            がカード下部に折れ線で示されます（センサー実測と異なることがあります）。環境省の目安区分に応じた色の帯、現在までを実線・この先を破線、予測期間内のみ「現在」の縦線で区切る表示です。{' '}
            <strong>気象庁の地域ヒート注意情報はダッシュボードのカードには出しません。</strong>
            詳細画面では、その予測表を実測の推移グラフや時間別履歴の
            <strong>下側</strong>に配置し、そのほか気象庁の注意情報を別欄として表示することがあります。デモ構成ではデモ表示が混ざる場合があります。
          </p>
          <ul className="list-disc list-outside pl-5 space-y-2">
            <li>
