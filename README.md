...
**ブラウザ受け入れ（Playwright、`frontend` をビルド済みであること）：**

```bash
cd frontend && npm ci && npm run build
cd ../tests && npm ci && npx playwright install chromium && npm run test:functional
```

Linux CI では `npx playwright install --with-deps chromium` を利用します（`.github/workflows/ci.yml` の `acceptance` ジョブと同様）。

**受け入れ（CI と同様の一括）:** リポジトリルートで `.\scripts\run-acceptance-tests.ps1` または `npm run test:acceptance`（`frontend` の build / test、`functions` の seed-test-data `--dry-run` と test。続けて `tests/` があれば `npm ci` のうえ `test:functional` と `test:e2e` を `--if-present` 実行）。

CI（`main` 向け）は `.github/workflows/ci.yml` で `frontend` の `build` / `test`、`functions` の `test`、および `tests/` の Playwright（`acceptance` ジョブ）を実行します。
